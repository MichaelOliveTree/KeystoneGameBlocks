...
namespace Keystone.Entities
{
    ...
    public abstract class Entity : BoundTransformGroup, IFXSubscriber, IPageableTVNode
    {
        ...
        System.Memory.Memory<Keystone.Data.IntrinsicDataStores.AdvancedEntityData> mAdvancedEntityData;
        // <summary>
        /// IntrinsicData is created and assigned by Scene.EntityAttached() via 
        /// mComponentStoreCollection.CheckOut<Keystone.Data.IntrinsicDataStores.AdvancedEntityData> (entityID);
        /// </summary>
        internal System.Memory.Memory<Keystone.Data.IntrinsicDataStores.AdvancedEntityData> IntrinsicData
        {
            get {return mAdvancedEntityData;}
            set { mAdvancedEntityData = value;} // this should only be assigned via Scene.EntityAttached()
        }
        
        /// <summary>
        /// BlackboardData is created and assigned by Scene.EntityAttached() via mUserDataStore.CheckOut(entityID);
        /// Blackboard data is used by Scripts to store runtime data that is needed during gameplay but which does
        /// not need to be serialized.
        /// It is also used by KeyCommon.Data.Processors for efficient updating of the simulation.
        /// </summary>
        ...
        private Dictionary<string, KeyScript.Events.EventSubscribers> mEvents;
        ...
        protected override void PropogateChangeFlags(Keystone.Enums.ChangeStates flags, Enums.ChangeSource source)
        {
            ...
            Keystone.Enums.ChangeStates filter =
                Keystone.Enums.ChangeStates.NotifyModelTransformed |
            ...
            Keystone.Enums.ChangeStates foundFlags = flags & filter; // = flags & ~filter;
            ...
            if (foundFlags != 0)
            {
                if ((foundFlags & Keystone.Enums.ChangeStates.NotifyModelTransformed) !=0)
                {
                    ...
                    {
                        // source is a Child so is the actual Model itself notifying the Entity it's hosted by
                        NotifyParents(Keystone.Enums.ChangeStates.NotifyModelTransformed);
                        NotifyChildOfTypeEntitesOnly(Keystone.Enums.ChangeStates.NotifyModelTransformed);
                    }
                    else if (source == Enums.ChangeSource.Parent)
                    {
                        // source is a Parent Entity of THIS Entity and is so, also, the Parent of a Model that has been Transformed
                        // to which THIS Entity may have AttachedToSubModelIndex > -1
                        NotifyChildBoundTransformNodes(Keystone.Enums.ChangeStates.NotifyModelTransformed);
                        // Note: we do NOT notify parent Entities here because the source is already a parent
                    }
                ...
                if (mChildren != null && (source == Enums.ChangeSource.Parent || source == Enums.ChangeSource.Self))
                    NotifyChildEntities(foundFlags);
                ...
                if (mSceneNode != null)
                {
                    ...
                    foundFlags = flags & filter; // foundFlags = flags & ~filter;
                    if (foundFlags != 0)
                    {
                        ...
                        mSceneNode.SetChangeFlags(foundFlags, Enums.ChangeSource.Target);   //"Target" refers to a SceneNode's Entity target as opposed to it's actual Child node.
      ...
      protected virtual void NotifyChildrenOfTypeEntitiesOnly(Enums.ChangeStates flags)
        {
            // this function is called by this class only when a NotifyModelTransformed change flag is handled
            ...
            for (int i = 0; i < children.Length; i++)
                // TODO: verify Container handles this method properly.  Untested along with any overrides it has for PropogateChangeFlags()
                // NOTE: Container overrides this Method to avoid notifying a Region child
                //       since region's have their own coordinate systems that are unaffected by
                //       a parent's transform change.
                if (children[i] is Entity)
                    children[i].SetChangeFlags(flags, Enums.ChangeSource.Parent);
        }
        
        // NOTE: this is also called in the BonedModel.Update() for handling child models attached to bones <-- todo: needs testing
        /// BoundTransformGroup include Entities, ModelSelectors and Models
        protected virtual void NotifyChildBoundTransformNodes(Enums.ChangeStates flags)
        {
