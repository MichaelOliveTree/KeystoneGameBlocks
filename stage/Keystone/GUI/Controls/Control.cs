...
2. Get the MethodInfo using reflection
...
// Get the MethodInfo for the instance method (requires an instance).
...
// a specific source eg.  my.Exe\GUIHandlers.cs  
    ...
    // NOTES ON RENDERING
    // =====================
    // if Model.Render() has a Mesh type that is a "BufferedGeometry.cs" for Instance
    // then we are pushing the "quadIndex, translation, scale, rotation and UV" info to a buffer 
    //  that is shared by all GUI controls.  MOST GUI elements dont change from frame to frame source
    //  there's no reason to update them.
    //  Our "bufferedGeometry" can host a single TVMesh and each quad can be manipulated via it's "quadIndex".
    //  I think this is worth testing and not just using TVMinimesh since I think with TVMinimesh we can't 
    //  modify the UV coords of different instances by default.
    // =====================
