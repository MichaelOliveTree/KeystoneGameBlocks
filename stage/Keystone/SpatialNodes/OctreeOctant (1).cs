...
namespace Keystone.Octree
{
    ...
    /// <summary>
    /// A dynamic octree implementation. "Loose" octree effect is accomplished during Cull traversal 
    /// by 
    ...
    public class OctreeOctant : ISpatialNode, ITraversable, IBoundVolume
    {
        ...
        public string Address { get {return OctreeOctant.GetNodeAddress(this);}}
        
        public static string GetNodeAddress (ISpatialNode node)
        {
            string result = node.Index;
...
//            this.BoundingBox.Max.z - this.BoundingBox.Min.z);
            ...
            octantRadius /= 2d; // before this divide, we are actually Distance.  So half Distance gives us OctantRadius
            ...
            {
            	// if this bitflag combination is not set
                if (code != i) continue;
