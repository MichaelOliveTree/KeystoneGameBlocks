...
namespace Keystone.Simulation
{
    ...
    {        
        ...
        private double _elapsedMS;    // elapsed for current frame
        private long mTicks;
        ...
        public double Elapsed
        {
            get
            {
                ...
                double elapsedMS = (double)CoreClient._CoreClient.Engine.AccurateTimeElapsed();
        ...
        public float GetJulianDay (DateTime time)
        {
            ...
            float jd = time.DayOfYear + (153 * m + 2) / 5 + y * 365 + y / 4 - y / 100 + y / 400 - 32045;
            jd -= 2442414;
            jd -= 1f / 24f;
        ...
        // TODO: this should grab it's own elapsedSeconds via this.ElapsedSeconds
        public void Update(double elapsedMS)
        {
        	if (_timeScaling == 0.0f) return; 
        	
            _elapsedMS = elapsedMS * _timeScaling;
            _totalElapsed += _elapsedMS;

            _time = _time.Add(new TimeSpan(0, 0, 0, 0, (int)_elapsedMS));
            mTicks = _time.Ticks; 
        }
    }
}